...
def add_usholiday_features(df: pd.DataFrame, date_col: str):
    """Add US Holiday features to the data frame

     Args:
        df (pd.DataFrame): The dataframe which has the date info which has to be encoded
        date_col (str): date column name

    Returns:
        [Tuple[pd.DataFrame, List]]: Returns a tuple of the new dataframe and a list of features which were added
    """
    first = str(df[date_col].dt.date.min())
    last = str(df[date_col].dt.date.max())
    ...
    # np where with isin
    df["Holiday"] = np.where(
        df.set_index(date_col).index.to_period("D").isin(hol), 1, 0
    )
    return df, ["Holiday"]
